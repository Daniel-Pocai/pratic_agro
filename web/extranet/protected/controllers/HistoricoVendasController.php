...
class HistoricoVendasController extends GxController {

	
    public function getRepresentingFields(){
		return array('data_venda', 'representante', 'cliente' );
	}
    
	public function actionView($id) {
		$this->render('view', array(
			'model' => $this->loadModel($id, 'HistoricoVendas'),
		));
	}

	public function actionCreate() {
		$model = new HistoricoVendas;
        //Registro do model no log
		$this->model = $model;
		if (isset($_POST['HistoricoVendas'])) {
			$model->setAttributes($_POST['HistoricoVendas']);

			if ($model->save()) {
				if (Yii::app()->getRequest()->getIsAjaxRequest())
					return true;
				else
					$this->redirect($this->createUrlRel('view',array('id' => $model->idvenda,'success'=>'create')));
			}
		}
        else{
	...
	public function actionUpdate($id) {
		...
		if (isset($_POST['HistoricoVendas'])) {
			...
			if ($model->save()) {
                $this->redirect($this->createUrlRel('view',array('id' => $model->idvenda,'success'=>'update')));
			}
		}
	...
	public function actionDelete($id) {
        if($_GET['confirm'] == 1){		
			...
			if(count($explode_ids) > 1){
				...
				$model = $this->loadModel($explode_ids[0], 'HistoricoVendas');
	...
	public function actionIndex() {
       ...
       if(isset($_GET['o']) && isset($_GET['f']) ){
		...
		$dataProvider = new CActiveDataProvider('HistoricoVendas', array(
            'criteria'=>$criteria,
			'pagination' => array(
	...
	public function actionFiltrarClientes() {
		if (isset($_POST['idrepresentante'])) {
			$idrepresentante = $_POST['idrepresentante'];
			$clientes = Cliente::model()->findAllByAttributes(array('idrepresentante' => $idrepresentante));
			echo CHtml::tag('option', array('value' => ''), 'Selecione...', true);
			foreach ($clientes as $cliente) {
				echo CHtml::tag('option', array('value' => $cliente->idcliente), CHtml::encode($cliente->nome), true);
			}
		}
	}
