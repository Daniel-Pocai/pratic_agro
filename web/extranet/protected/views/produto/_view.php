...
?><tr data-id="<?=$data->getPrimaryKey()?>" id="listItem_<?= $data->getPrimaryKey()?>">
  <?
    ...
    foreach($this->getRepresentingFields() as $field){
        ...
        <td data-title="<?=$data->getAttributeLabel($field);?>">
            ...
            if($field == 'descricao'){
                ...
                $imageUrl = '/../pratic_agro/web/extranet/uploads/Produto/' . basename($data->foto);
                if(!empty($data->foto)){
                    echo '<img src="' . $imageUrl . '"  width="150" alt="Foto" onerror="this.onerror=null; this.src=\'/extranet/uploads/Produto/default-image.jpg\';" />';
                } else {
                    echo '<img src="/extranet/uploads/Produto/default-image.jpg" alt="Foto não disponível" />';
                }
            } else {
