...
?><tr data-id="<?=$data->getPrimaryKey()?>">
  ...
  foreach($this->getRepresentingFields() as $field){
    ...
    <?php 
    if($field == 'foto'){
        $imageUrl = '/pratic_agro/web/extranet/uploads/Representante/' . basename($data->foto);
        if(!empty($data->foto)){
            echo '<img src="' . $imageUrl . '"  width="150" alt="Foto" onerror="this.onerror=null; this.src=\'/extranet/uploads/Representante/default-image.jpg\';" />';
        } else {
            echo '<img src="/extranet/uploads/Representante/default-image.jpg" alt="Foto não disponível" />';
        }
    } else {
