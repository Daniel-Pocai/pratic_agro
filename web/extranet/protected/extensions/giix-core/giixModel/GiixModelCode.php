...
class GiixModelCode extends ModelCode {
	...
	public function prepareTable($table){
		...
		try{
			...
			$command->addColumn($table->name,'habilitar',"CHAR(1) NOT NULL DEFAULT '0' AFTER ".$table->primaryKey);
	...
	protected function getRepresentingColumn($table) {
		...
		if (!$this->isRelationTable($table)) {
			...
			foreach ($columns as $name => $column) {
				if ($column->name != 'habilitar' && $column->type === 'string' && !$column->allowNull && !$column->isPrimaryKey && !$column->isForeignKey && stripos($column->dbType, 'int') === false)
					return $name;
			}
			// Then a string, not null, not fk column, not original number on db.
			foreach ($columns as $name => $column) {
				if ($column->name != 'habilitar' && $column->type === 'string' && !$column->allowNull && !$column->isForeignKey && stripos($column->dbType, 'int') === false)
					return $name;
			}
			// Then the first string column, not original number on db.
			foreach ($columns as $name => $column) {
				if ($column->name != 'habilitar' && $column->type === 'string' && stripos($column->dbType, 'int') === false)
					return $name;
			}
