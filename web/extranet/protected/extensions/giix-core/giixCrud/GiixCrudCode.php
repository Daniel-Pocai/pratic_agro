...
class GiixCrudCode extends CrudCode {
	...
	public function generateActiveField($modelClass, $column) {
		...
		elseif($column->size == 160){
			return "\$this->widget('application.extensions.PluploadImagem.PluploadImagem', array(
			'model' => \$model,
			'attribute' => '{$column->name}',
	...
	public function generateDetailView($modelClass, $column) {
		...
		if (!$column->isForeignKey) {
			...
			elseif($column->size == 160)
				return '<img src="<?php echo Yii::app()->createAbsoluteUrl(\'thumbnail/fill/250/'.$modelClass.'/\'.$model->'.$column->name.');?>"/>';
			else
