	...
	private function criaImagem()
	{
		switch ( $this->formato )
		{
			...
			case 3:
				...
				break;
			case 6:
				$this->img = imagecreatefrombmpcustom( $this->origem );
				$this->extensao = 'bmp';
				break;
			default:
...
// funções para redimensionamento
	...
	*/
	private function resizeFill()
	{
		...
		//correção crop no fill
		if($this->nova_altura > $dif_y){
			$this->nova_largura	= ( ( $this->largura * $dif_y ) / $this->altura );
			$this->nova_altura	= ( ( $this->altura * $this->nova_largura ) / $this->largura );
		}
		...
		if ( $imagem ) {
			...
			switch( strtolower( $pathinfo['extension'] ) )
			{
				...
				case 'gif':
					...
					break;
				case 'bmp':
					$marcadagua = imagecreatefrombmpcustom( $imagem );
					break;
				default:
...
/*********************************************/
/* Function: ImageCreateFromBMPcustom              */
/* Author:   DHKold                          */
...
function imagecreatefrombmpcustom($filename) {
